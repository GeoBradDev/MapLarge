...
namespace MapLarge.Controllers
{
    ...
    public class BrowseController : ControllerBase
    {
        ...
        public async Task<IActionResult> UploadFile([FromRoute] string? path, IFormFile? file)
        {
            ...
            var targetDir = Path.Combine(_rootDir, path ?? "");
        ...
        // GET /api/browse/download/{*path} - Downloads a file from the server
        // Returns the file at the specified path (relative to rootDir) as a downloadable response
        // If the path points to a directory, returns a ZIP archive of the directory contents
